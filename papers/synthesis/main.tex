...
with a small residual over the measured range. At the structured level the natural
...
tool must satisfy, the controls it must fail, and ten open problems --- together
with one question, asked in an earlier version of this note, that is retracted
here as false.
...
rate, an exact polynomial correction, and record-holding integers that sit, over
the computed range, where random placement at that rate predicts --- and the
\emph{structured} candidates are excluded one family at a time: periodic,
algebraic, Sturmian, bounded-drift, and descent-accessible. What is left is
neither a counting problem nor a structural one. It is a question about where
particular integers lie inside a population whose size is already known, and
the tools that decided every other part of the programme are provably unable
to see it. Section~\ref{sec:needed} is an attempt to say precisely what a tool
that could see it would have to do.
...
that at the population level, and --- over the computed range --- at the extreme
order statistic, the integers behave like random placement at the proved rate.
...
\begin{remark}[what the record-holder data actually measures]\label{rem:decomp}
Write $p_N=p_N(0)$ and put
\[
A_N:=p_N\,2^{\,I_0N}N^{3/2},\qquad
\Delta_N:=\log_2\!\bigl(r_{\min}(N,0)\,p_N\bigr).
\]
Then, identically,
\begin{equation}\label{eq:decomp}
\log_2 r_{\min}(N,0)\;=\;I_0N+\tfrac32\log_2 N-\log_2 A_N+\Delta_N .
\end{equation}
Theorem~\ref{thm:scales} is the statement that $A_N$ is bounded above and below:
it \emph{is} the $O(1)$ phase, renewal functions, Beatty jump sets and all. So the
first three terms of \eqref{eq:decomp} are the population count and $\Delta_N$ is
everything else.
\end{remark}
\lab{proved (paper)} for \eqref{eq:decomp}, which is arithmetic. The point of
writing it out is that the record-holder audit compares $\log_2 r_{\min}(N,0)$
against $\log_2(1/p_N)$ computed \emph{exactly}, by the transfer recursion of
Proposition~\ref{prop:density}, and not against the asymptotic
$I_0N+\frac32\log_2N$. Its residual is therefore exactly $\Delta_N$, and $A_N$ is
absorbed into the comparison and never enters: \textbf{no part of what
Proposition~\ref{prop:clustering} or Conjecture~\ref{conj:sharp} measures depends
on the renewal phase.} What is measured is that $\Delta_N>0$ at all $24$ record
holders and drifts by no amount the data can distinguish from zero.
...
gives $93$ record holders to depth $459$, the deepest
$r_{\min}(459)=848\,537\,873\,557$. As for $3x+1$, $\Delta_1>0$ at nearly every
holder ($92$ of $93$; the exception is $N=13$, $\Delta_1=-0.40$). Where $3x+1$
shows no trend, the $5x-1$ data \emph{appear} to grow: least-squares slope
$+0.0224$ per unit $N$, Theil--Sen $+0.0224$, and block-bootstrap $95\%$ intervals
excluding zero at block lengths $3$, $8$ and $16$. The identical pipeline on
$3x+1$ over $m<2\cdot10^{11}$ returns $23$ holders, $\Delta_1=2.19$ bits at
$N=282$, slope $-0.0033$, every interval containing zero, and growth exponent
$0.076$ --- all consistent with the values behind Conjecture~\ref{conj:sharp},
which rest on a scan five times longer, so the instrument is calibrated and the
contrast between the two columns is measured rather than assumed.

\textbf{That contrast is suggestive only, and the reason is the sample.} The
$93$ rows are not $93$ independent observations: consecutive depths share a
holder, the lag-one autocorrelation of $\Delta_1$ is $0.94$, and
\textbf{the effective sample size is $n_{\mathrm{eff}}\approx3.0$} ($6.3$ for
$3x+1$). At that effective size the bootstrap intervals carry far less
information than their width suggests, and a growing $\Delta_1$ and a bounded one
are not separated by this data. Nothing below rests on the trend.

\emph{These $5x-1$ figures are from a rescan of 2026-09-25.} The first version of
this remark reported $66$ holders, $66$ of $66$ positive, and slope $+0.0211$,
from a scan that sieved to $m\equiv5\ (\mathrm{mod}\ 16)$ --- half of the seeds
admissible at the first step, which are $m\equiv5\ (\mathrm{mod}\ 8)$. The
correction moved every $5x-1$ number and did not reverse the direction of the
apparent trend --- which, given $n_{\mathrm{eff}}\approx3.0$, is not itself
evidence for the trend. The $p_N$ column did not move at any $N$, the exact mass
recursion never having been involved, and the $3x+1$ column is unaffected and was
re-verified. See \texttt{audits/cross\_map/CORRECTION\_2026-09-25.md}.
\end{remark}
...
\medskip
\noindent\textbf{0. The status of the comparison problem, and which statement the
analogy uses.} \lab{cited} throughout; no position is taken here on any of it. On
8~September 2026 a manuscript \cite{OpenAINS2026} constructed, for every $\nu>0$,
a force $f\in C_c^{\infty}(\R^3\times(0,\infty);\R^3)$ and smooth $u,p$ on
$\R^3\times[0,1)$ with
\[
u(\cdot,0)=0,\qquad \sup_{0\le t<1}\|u(t)\|_{L^2(\R^3)}<\infty,\qquad
\limsup_{t\uparrow1}\|u(t)\|_{L^{\infty}(\R^3)}=\infty,
\]
stating that this establishes alternatives \textup{(C)} and \textup{(D)} of
Fefferman's official problem description \cite{Fefferman2006}; a Lean
formalization accompanies it \cite{OpenAINSLean}. On 11~September 2026 the Clay
Mathematics Institute stated that the problem ``has apparently been settled'',
adding that the evaluation process ``is deliberately unhurried'' \cite{CMI2026}.
No prize has been awarded and the evaluation continues.

\textbf{None of this touches the statement the comparison below uses.}
Fefferman's alternatives \textup{(A)} and \textup{(B)}, existence and smoothness,
say ``Take $f(x,t)$ to be identically zero'' \cite{Fefferman2006}, while the
construction requires a nonzero force. The \emph{unforced} global regularity
question is therefore untouched by it and remains open, and it is with that
unforced problem --- and only with it --- that the analogy is drawn.
...
at the proved rate of Theorem~\ref{thm:rate} predicts, with a small residual over
the measured range. What it does not do: it proves nothing whatever about
...
\medskip
\noindent\textbf{5. The Navier--Stokes comparison.} \lab{analogy}, with the
unforced problem of \textup{(A)}/\textup{(B)}. The two problems have accumulated
the same four kinds of statement, in the same order, and are missing the same kind.

One structural difference should be stated first, because it is where the analogy
is weakest. The forced construction of \cite{OpenAINS2026} gets to \emph{choose}
an external force, and choosing it is most of the work; alternatives \textup{(C)}
and \textup{(D)} grant exactly that freedom. The accelerated $3x+1$ map is fixed.
There is no forcing term to select here, hence no counterpart of that freedom and
no counterpart of the construction that uses it --- which is a further reason the
comparison is drawn with the unforced statement.
...
length. \lab{open}.
\item \textbf{Predicting $\Delta_N$.} Give a formula, or a model, for the residual
$\Delta_N$ of Remark~\ref{rem:decomp} in terms of \emph{order-sensitive} carry
statistics of the confined words --- statistics that do not take $r_{\min}$ as an
input --- and test it at finite depth against the exact $p_N$ of
Proposition~\ref{prop:density}. \lab{open}. This is a finite-depth question:
every ingredient is computable at each $N$ and none of it requires an asymptotic
statement, which is what distinguishes it from the rest of this list. Two cautions
belong with it. First, the sample is small and strongly correlated --- $24$ record
holders over $N\le346$ with $n_{\mathrm{eff}}\approx7$, and for $5x-1$ (which is
not $3x+1$, but is the only other ladder measured) $93$ over $N\le459$ with
$n_{\mathrm{eff}}\approx3.0$, Remark~\ref{rem:null5} --- so a model that merely
fits the observed $\Delta_N$ is evidence of very little. Second, and decisively: a
formula alone would not bear on \textup{(DE)}. Since
$\log_2 r_{\min}(N,0)=\log_2(1/p_N)+\Delta_N$ with $\log_2(1/p_N)\to\infty$, only
a \emph{uniform lower bound} $\Delta_N\ge-c$ would give $r_{\min}(N,0)\to\infty$,
hence \textup{(DE)} by Proposition~\ref{prop:equivalent}\,(b) --- and proving such
a bound is therefore at least as hard as \textup{(DE)} itself.
\item \textbf{\textup{RETRACTED}: the $5x-1$ residue law.} An earlier version of
this note asked whether $r_{\min}(N)\equiv5\ (\mathrm{mod}\ 16)$ for the
upper-side frontier of $T_{5,-1}$, and recorded it as \lab{verified} at every
$N\le459$. \textbf{The statement is false, and the supporting computation was
defective.} It fails already at $N=2$: $5\cdot13-1=64$, so $S_1=6\ge3$ and
$m=13$ is rare-side persistent, whence $r_{\min}(2)=13\equiv13\
(\mathrm{mod}\ 16)$, not the $21$ that was reported.

The cause is a sieve. For $q>4$ the condition at $n=1$ is $S_1\ge A[1]+1$, a
\emph{lower} bound, so every first letter $d\ge A[1]+1$ is admissible and the
seeds form one class modulo $2^{A[1]+1}$ --- for $T_{5,-1}$ exactly the
$m\equiv5\ (\mathrm{mod}\ 8)$ that the previous wording itself derived. The
scanner sieved instead to the single class with $v_2(5m-1)=3$ \emph{exactly},
$m\equiv5\ (\mathrm{mod}\ 16)$, and so never visited $m\equiv13\
(\mathrm{mod}\ 16)$. The clause "the class $13$ is admissible yet never least"
was the artefact of that omission. A complete rescan of every odd $m<10^{12}$
finds $93$ record holders in place of $66$, of which $66$ --- a clear majority ---
lie in the class $13\ (\mathrm{mod}\ 16)$, and $13$ is itself least at $N=2$.
\lab{verified} for the refutation; the residue question is \lab{open} again, with
no empirical pattern behind it. What survives unchanged is the \emph{proved}
partial constraint $S_1\ge3\Rightarrow m\equiv5\ (\mathrm{mod}\ 8)$, and the
observation that the $3x+1$ analogue (L4) of Theorem~\ref{thm:descent} is proved
\emph{through} (L1), which has no counterpart here: for $q>4$ the
persistence-preserving backward step multiplies the seed by
$2^{\lceil\alpha_q\rceil}/q>1$ instead of contracting it, so the descent route to
a residue law is unavailable.
\end{enumerate}
...
alignment of Lemma~\ref{lem:signalign}; \texttt{audits/cross\_map}, which
...
record scan; and \texttt{audits/pointwise\_discovery} (verdict \textsc{d}, no
usable pointwise signal), which measured the finite-height question of
Observation~\ref{obs:integer} directly and found nothing beyond
Terras--Everett, and whose reproduction of the controls exposed the $5x-1$ sieve
defect. Each carries its own report with every claim labelled and every
computation scripted.

\medskip
\noindent\textbf{One correction to a published control.} The $5x-1$ record ladder
of \texttt{audits/cross\_map} was computed over a sieved population omitting half
of the admissible seeds; it was wrong from $N=2$, and \emph{the residue question
that rested on it is retracted in Section~\ref{sec:open}, item~11}. The scanner is
fixed, the full range rescanned, every derived statistic regenerated, and a
regression test added; see \texttt{audits/cross\_map/CORRECTION\_2026-09-25.md}.
The $3x+1$ ladder is unaffected --- for $q<4$ the same sieve is exact --- and this
