...
| 12 | `N^{−3/2}` two-sided | PROVED (paper) | *Three Scales of Confinement*, Zenodo `10.5281/zenodo.22663600` |
| 12b | renewal phase; Beatty jump sets; no priority claim vs Banderier–Wallner | PROVED (paper) — **version to confirm, §6** | same, *second revision* (local PDF) |
...
## 6. DOIs received — and one discrepancy to resolve before drafting

**Received, and entered into the ledger (rows 2 and 12):**

* De Jesús, Elias (2026). *Three Scales of Confinement in Accelerated Collatz Words: Entropy,
  Ballot Persistence, and **Sturmian Phase***. Zenodo.
...
### The discrepancy, and why it is load-bearing

The **title you gave for the first ends "and Sturmian Phase". The PDF I read ends "and an
Irrational Renewal Phase"** (`~/Downloads/ScalesConfinement (1).pdf`, titled *Technical Note,
second revision*, September 2026). That is not a transcription slip on either side: the second
revision's own Remark 3.2 says

> "As explained in §8.4, the `O(1)` renewal phase identified **in this revision** is a different
> arithmetic object, generated by the **Beatty sequences of `β`** rather than by this mechanical
> clock, though both are driven by the irrationality of `α`."

So the Scale-3 object appears to have been **replaced** between versions, and the version at
`10.5281/zenodo.22663600` may carry the older *Sturmian phase* rather than the renewal phase.

**Theorem 4.5 as drafted in §2 above cites the renewal functions `U, Û` and the Beatty jump
sets — i.e. the second revision.** Before drafting I need one of:

1. the second revision **is** the current version at that DOI (the concept DOI resolves to it),
   and the Zenodo title is simply stale — then I cite the DOI and use the PDF's title, noting
   the version; or
2. the second revision is **not yet deposited** — then Theorem 4.5 splits: the `N^{−3/2}`
   two-sided asymptotic is **PROVED (paper)** and citable at
   `10.5281/zenodo.22663600`, while the renewal phase and the Beatty jump sets are
   **PROVED (paper), in preparation**, cited as an unpublished second revision; or
3. you give me the version DOI of the second revision.

I cannot check Zenodo myself — web search is off for this task — so this needs a line from you.

*(Related and already handled: the second revision's §12.2 records that Banderier and Wallner
announced "Lattice paths below a line of irrational slope" as work in preparation and that no
posted version was located, so **no priority claim** is made for the renewal-phase result. The
note will carry that caveat verbatim wherever Theorem 4.5's Scale 3 is mentioned.)*

## 7. Still open from Checkpoint 1

**The title.** A, B, C or D from §0 — I recommend **A**. Not yet chosen.
