...
bound for every irrational $\gamma\in(0,1)$; below $\ln 2/\ln 3$ irrationality
was known (Monks--Yazinski), above it the only previous argument is
unpublished.
\end{abstract}
...
effective proof in place of an unpublished archimedean one. All of this stays at
intercept $0$. It says nothing about divergent orbits of positive integers,
...
\begin{enumerate}
\item \emph{General intercepts.} For $\rho\in[0,1)$ let
$1c_{\gamma,\rho}(j)=\lceil (j+1)\gamma+\rho\rceil-\lceil j\gamma+\rho\rceil$.
Is $\Ph(1c_{\gamma,\rho})$ irrational for every irrational $\gamma\in(0,1)$ and
every $\rho$? Section~\ref{sec:allslopes} settles $\rho=0$ only, and the
obstruction is precise. The ones of $1c_{\gamma,\rho}$ no longer sit at
$\lfloor j\alpha_\gamma\rfloor$, so the quantity
$m_j=jq_n\bmod p_n$ of Lemma~\ref{lem:floor} acquires a shift, and with it the
two exact identities on which that proof turns --- $m_{p_{n+1}}=p_n-1$ from
$p_{n+1}q_n\equiv-1\pmod{p_n}$, and
$(p_n+p_{n+1})\alpha_\gamma D_n=1+p_n\alpha_\gamma(D_n-D_{n+1})$ --- lose their
right-hand sides. The first mismatch index $j^{*}_n$ is then no longer
$p_n+p_{n+1}$, and there is at present no replacement for the depth law
$q_n+q_{n+1}-1$. The height bound (Lemma~\ref{lem:height}) and the Liouville
comparison (Theorem~\ref{thm:allmain}) survive unchanged, so a depth law at
general intercept would close this case immediately.
...
None of these, and neither Theorem~\ref{thm:main} nor
Theorem~\ref{thm:allslopes}, bears on divergent orbits of positive integers, on
the Periodicity Conjecture in general, or on the Collatz conjecture. The
Periodicity Conjecture quantifies over all aperiodic parity vectors; the words
$1c_\gamma$ form a one-parameter family among them, and outside the critical
density the conclusion for that family was already available
