...
\subjclass[2020]{Primary 11B83; Secondary 11J72, 68R15, 11A55}
...
\cite{LopezStoll2009} studied $\Ph$ on Sturmian parity words, produced a $2$-adic
series and a generalised continued fraction for the values, computed examples
suggesting full complexity of the images, and wrote that they had no proof.
...
the $3x+1$ map: a parity word of ones-density $\beta$ describes an orbit whose
size is asymptotically neither growing nor shrinking.
...
later work \cite{LopezStoll2021} proves aperiodicity of the parity words for
ones-density \emph{strictly} above $\beta$, and its Theorem~1 shows that a
divergent trajectory of a rational $2$-adic integer would have ones-density
exactly $\beta$. The word of Theorem~\ref{thm:main-intro} sits at that single
...
\emph{(2), $n$ even.} Here $\eta_n>0$, so a mismatch at $j$ means
...
\eqref{eq:ident} lies in $(1,2)$. In particular $p_{n+1}\alpha D_n<1$.

Now no $j\le p_{n+1}$ satisfies \eqref{eq:mis}: since $m_j\le p_n-1$, it would
force $j\alpha D_n\ge1$, whereas $j\alpha D_n\le p_{n+1}\alpha D_n<1$. Next, for
...
\[
|M_n|\le |u|\,|\delta_n|+v\,c_n\le H\bigl(3^{p_n}+q_n3^{p_n}\bigr)
