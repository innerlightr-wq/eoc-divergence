...
int main(int argc, char **argv) {
    if (argc != 5) { fprintf(stderr, "usage: %s <q> <r:+1|-1> <lower|upper> <X>\n", argv[0]); return 2; }
    int qq = atoi(argv[1]); int rr = atoi(argv[2]); int up = (argv[3][0] == 'u');
    u64 X = strtoull(argv[4], 0, 10);
    int q = qq; long long r = rr; int upper = up;
    build_A(q);
    ...
    printf("q=%d r=%+lld  %s side  X=%llu\n", q, r, upper ? "UPPER (anti-confined)" : "LOWER (confined)", X);
