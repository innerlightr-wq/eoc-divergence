 ...
 * Output: "A N c m1 m2 m3" -- the c<=3 smallest N-confined m in this range;
 *         "R N c m1 m2 m3" -- the same restricted to CHAIN ROOTS, i.e. m % 3 != 2.
 *
 * Roots.  By L1/L2, a confined m with m = 2 (mod 3) has a smaller, more-confined
 * backward d=1 image, and the chain from m has length exactly v3(m+1); its bottom
 * is the root, characterised by 3 does not divide m+1, i.e. m != 2 (mod 3).  Since
 * 2^{S+1} is invertible mod 3, exactly 2/3 of each residue class mod 2^{S+1} are
 * roots, so the root density is exactly (2/3) p_N(0).
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static u64 best[KMAX + 1][3], bestR[KMAX + 1][3];
static int cnt[KMAX + 1], cntR[KMAX + 1];
...
int main(int argc, char **argv) {
    ...
    memset(cnt, 0, sizeof cnt); memset(cntR, 0, sizeof cntR);
    ...
    for (u64 m = start; m < end; m += 4) {
        ...
        for (int N = 1; N <= k; N++) {
            ...
            if (isroot && cntR[N] < 3) bestR[N][cntR[N]++] = m;
        }
    }
    for (int N = 1; N <= maxdepth; N++) {
        if (cnt[N]) {
            printf("A %d %d", N, cnt[N]);
            for (int i = 0; i < cnt[N]; i++) printf(" %llu", best[N][i]);
            printf("\n");
        }
        if (cntR[N]) {
            printf("R %d %d", N, cntR[N]);
            for (int i = 0; i < cntR[N]; i++) printf(" %llu", bestR[N][i]);
            printf("\n");
        }
    }
    return 0;
