/* Exact record-holder scan (SGN = +1: 3x+1;  SGN = -1: 3x-1, the sign control) for the accelerated 3x+1 map.
 *
...
int main(int argc, char **argv) {
    ...
    if (start < 1) start = 1;
    int R0 = (SGN > 0) ? 3 : 1;                  /* 2^{S_1} <= 3 forces v2(3m+SGN) = 1 */
    start += (R0 + 4 - (start & 3)) & 3;
